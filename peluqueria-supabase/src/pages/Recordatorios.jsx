...
import { getTurnos, actualizarEstado } from '../api.js'
...
export default function Recordatorios() {
  ...
  useEffect(() => {
    getTurnos(hoy).then(data => setTurnos(data.filter(t => ['pendiente','confirmado'].includes(t.estado))))
  }, [])
  ...
  const copiarMensaje = (t) => {
    const msg = `Hola ${t.cliente_nombre}! 💈 Te recuerdo tu turno hoy a las ${t.hora} hs para ${t.servicio}. ¡Te esperamos!`
  ...
  return (
    <>
      <div className="topbar">
        <div><h2>Recordatorios</h2><div className="topbar-sub">Turnos de hoy</div></div>
      </div>
      <div className="page-content" style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:560 }}>
        {pendientes.length > 0 && (
          <div className="card">
            <div className="card-header"><span className="card-title">Sin confirmar</span><span className="badge badge-amber">{pendientes.length}</span></div>
        ...
        {confirmados.length > 0 && (
          <div className="card">
            <div className="card-header"><span className="card-title">Confirmados hoy</span><span className="badge badge-green">{confirmados.length}</span></div>
